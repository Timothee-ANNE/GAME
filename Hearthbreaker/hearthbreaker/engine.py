...
class Game(Bindable):
    def __init__(self, decks, agents, seed, starting_player):
        super().__init__()
        random.seed(seed)  # fixed statting seed while still keeping the replications of the duel random (i.e., it's always the same random sequence of replications)
        self.delayed_minions = set()

        self.first_player = starting_player  # first player always starts to keep fair optimization red = first_player (formely: "self._generate_random_between(0, 1)")
